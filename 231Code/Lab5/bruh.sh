...
# Inspired by http://thoughtshubham.blogspot.com/2010/03/internet-over-usb-otg-on-beagleboa>

hostAddr=192.168.7.1
beagleAddr=${1:-192.168.7.2}

# Save the /etc/resolv.conf on the Beagle in case we mess things up.
ssh debian@$beagleAddr "mv -n /etc/resolv.conf /etc/resolv.conf.orig"
...
# From: http://askubuntu.com/questions/197036/how-to-know-what-dns-am-i-using-in-ubuntu-12-># Use nmcli dev list for older version nmcli
...
scp /tmp/resolv.conf debian@$beagleAddr:/etc

# Tell the beagle to use the host as the gateway.
ssh debian@$beagleAddr "/sbin/route add default gw $hostAddr" || true
